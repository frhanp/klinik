        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 md:p-8 bg-white border-b border-gray-200">
                    <x-notification />
                    <form method="POST" action="{{ route('pasien.biodata.update') }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <!-- Nomor Rekam Medis -->
                            <div>
                                <x-input-label for="nomor_rekam_medis" :value="__('Nomor Rekam Medis Anda')" />
                                <x-text-input id="nomor_rekam_medis" class="block mt-1 w-full bg-gray-100" type="text" name="nomor_rekam_medis" :value="$user->biodata->nomor_rekam_medis ?? 'Belum Diterbitkan'" readonly />
                            </div>
